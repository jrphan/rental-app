...
export const apiVehicle = {
  ...
  async getPopularVehicles(limit = 10): Promise<Vehicle[]> {
    ...
    if (response.success && response.data) {
      ...
      // Ensure we return a flat array of vehicles
      if (Array.isArray(data)) {
        // Check if it's a nested array and flatten if needed
        if (data.length > 0 && Array.isArray(data[0])) {
          return (data as Vehicle[][]).flat();
        }
  ...
  async getVehiclesByCity(city: string, limit = 20): Promise<Vehicle[]> {
    ...
    throw new Error(
      response.message || "Lấy danh sách xe theo thành phố thất bại"
    );
  },
