...
import { Text, View, FlatList, TouchableOpacity, Image, ActivityIndicator, Alert } from "react-native";
...
export default function MyVehiclesScreen() {
	...
	const handleSubmit = (vehicleId: string) => {
		Alert.alert("Gửi yêu cầu duyệt", "Bạn có chắc muốn gửi xe này để admin duyệt? Xe cần có ít nhất 1 hình ảnh.", [
	...
	const handleDelete = (vehicleId: string, vehicleName: string) => {
		Alert.alert("Xóa xe", `Bạn có chắc muốn xóa xe "${vehicleName}"? Hành động này không thể hoàn tác.`, [
			{ text: "Hủy", style: "cancel" },
			{
				text: "Xóa",
				style: "destructive",
	...
	const renderItem = ({ item }: { item: VehicleItem & { images?: { url: string }[] } }) => {
				...
				<View className="flex-row">
					...
					<View className="flex-1">
						<View className="flex-row items-center justify-between mb-2">
							...
							<View className="px-2 py-1 rounded" style={{ backgroundColor: statusBadge.bgColor }}>
								<Text className="text-xs font-medium" style={{ color: statusBadge.color }}>
									{statusBadge.label}
								</Text>
							</View>
						</View>
						...
						<Text className="text-sm text-gray-600 mb-1">Biển số: {item.licensePlate}</Text>
						...
						{item.status === "DRAFT" && (
							<View className="mt-2 flex-row gap-2">
								...
								>
									<Text className="text-gray-700 text-xs font-semibold">Chỉnh sửa</Text>
								</TouchableOpacity>
								<TouchableOpacity
						...
						{item.status === "SUBMITTED" && (
							<View className="mt-2 flex-row items-center">
								...
								<Text className="ml-1 text-xs text-gray-600">Đang chờ admin duyệt</Text>
							</View>
						)}
		...
		<SafeAreaView className="flex-1 bg-gray-50" edges={["top", "left", "right"]}>
			<View className="flex-1 px-6">
				<View className="flex-row items-center justify-between py-4">
					...
					<Text className="text-2xl font-bold text-gray-900 flex-1">Xe của tôi</Text>
					<TouchableOpacity onPress={() => router.push("/(tabs)/profile/vehicle-create")} className="ml-3">
				...
				{isLoading ? (
					...
					<View className="flex-1 items-center justify-center">
						...
						<Text className="mt-4 text-lg font-semibold text-gray-900">Chưa có xe nào</Text>
