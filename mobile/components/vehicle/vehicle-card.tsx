...
import { VehicleItem, vehiclesApi, VehicleType } from "@/lib/api.vehicles";
...
export function VehicleCard({
	...
	const vehicleTypeDescription = vehicleTypes.find((t: VehicleType) => t.id === vehicle.vehicleTypeId)?.description;
	...
	return (
		<View className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-3 shadow-sm">
			{/* Image Carousel */}
			<View className="w-full h-72 bg-gray-100 relative" onLayout={handleLayout}>
				{images.length > 0 ? (
					<>
						<ScrollView
							ref={scrollViewRef}
								...
								{images.map((_, index) => (
									<View
										...
										className={`mx-1 rounded-full ${
											index === currentIndex ? "bg-white w-2 h-2" : "bg-white/50 w-1.5 h-1.5"
										}`}
									/>
								))}
							</View>
			...
			{/* Content */}
			<TouchableOpacity onPress={handlePress} activeOpacity={0.7} className="p-4">
				{/* Header */}
				<View className="flex-row items-start justify-between mb-2">
					...
					{vehicle.status === "VERIFIED" && (
						<View className="flex-row items-center bg-green-50 px-2 py-1 rounded-full">
							...
							<Text className="ml-1 text-xs text-green-700 font-medium">Đã duyệt</Text>
						</View>
					)}
				</View>
				...
				<View className="mb-3">
					<View className="flex-row items-center mb-1">
						...
						<Text className="ml-2 text-sm text-gray-700">Biển số: {vehicle.licensePlate}</Text>
					</View>
              ...
              <Text className="ml-2 text-sm text-gray-700">
                  ...
                  : "Bán tự động"}
					...
					{vehicle.vehicleTypeId && (
						<View className="flex-row items-center mb-1">
							...
							<Text className="ml-2 text-sm text-gray-700">
								Loại xe: {vehicleTypeDescription ?? vehicle.vehicleTypeId}
							</Text>
						</View>
					)}
					{vehicle.location && (
				...
				<View className="flex-row items-center justify-between mb-3 pb-3 border-b border-gray-100">
					<View>
						...
						<Text className="text-xl font-bold text-orange-600">{formattedPrice} đ</Text>
					</View>
