...
export default function VehiclesScreen() {
  ...
  const renterRentals = useMemo(
    () => renterRentalsData?.rentals || [],
    [renterRentalsData?.rentals]
  );
  const ownerRentals = useMemo(
    () => ownerRentalsData?.rentals || [],
    [ownerRentalsData?.rentals]
  );
  const allRentals = useMemo(
    () => [...renterRentals, ...ownerRentals],
    [renterRentals, ownerRentals]
  );
  ...
  const vehiclesByStatus = useMemo(() => {
    ...
    vehicleStatusTabs.forEach((tab) => {
      if (tab.value === "ALL") {
        ...
        result[tab.value] = allVehicles.filter(
  ...
  const mappedAllRentals = useMemo(
    () => allRentals.map(mapApiRentalToRental),
    [allRentals]
  );
  const mappedOwnerRentals = useMemo(
    () => ownerRentals.map(mapApiRentalToRental),
    [ownerRentals]
  );
  const mappedRenterRentals = useMemo(
    () => renterRentals.map(mapApiRentalToRental),
    [renterRentals]
  );
