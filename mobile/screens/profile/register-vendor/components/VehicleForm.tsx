...
export default function VehicleForm({ vehicleId }: VehicleFormProps) {
  ...
  useEffect(() => {
    if (vehicleData) {
      const imageUrls = vehicleData.images
      ...
      form.reset({
        ...
        requiredLicense: vehicleData.requiredLicense as
          | "A1"
          | "A2"
          | "A3"
          | "A4",
      ...
      <View className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
        ...
        {conditions.map((condition, index) => (
          <View key={index} className="flex-row items-center mb-1">
            ...
            <Text
              className={`text-sm ${condition.completed ? "text-green-700" : "text-red-700"}`}
            >
              {condition.label}
            </Text>
          </View>
  ...
  const handleLocationSelect = (
    ...
    if (addressParts) {
      if (addressParts.fullAddress)
        form.setValue("fullAddress", addressParts.fullAddress);
      ...
      if (addressParts.district)
        form.setValue("district", addressParts.district);
      ...
      {vehicleData && vehicleData.status === "PENDING" && (
        <View className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4">
          ...
          <Text className="text-xs text-amber-700 mt-1">
            Bạn chỉ có thể xem thông tin. Không thể chỉnh sửa khi đang chờ
            duyệt.
          </Text>
        </View>
      )}
          ...
          return (
            <View className="mb-4">
              <Select
                ...
                disabled={
                  isReadOnly || !selectedBrand || modelOptions.length === 0
                }
              />
              {error && (
          ...
          <Controller
            ...
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <GalleryField
                ...
                value={value || ""}
                onChange={(url) =>
                  onChange(typeof url === "string" ? url : null)
                }
                error={error?.message}
              />
            )}
          ...
          <Controller
            ...
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <GalleryField
                ...
                value={value || []}
                onChange={(urls) =>
                  onChange(Array.isArray(urls) ? urls : urls ? [urls] : [])
                }
                error={error?.message}
              />
            )}
      ...
      {vehicleData && vehicleData.status === "PENDING" && (
        <View className="mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4">
          <Text className="text-sm font-medium text-blue-900 text-center">
            Xe của bạn đang chờ được duyệt. Vui lòng đợi phản hồi từ quản trị
            viên.
          </Text>
        </View>
      )}
