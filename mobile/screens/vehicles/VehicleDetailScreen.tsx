...
export default function VehicleDetailScreen() {
	...
	const deleteReviewMutation = useMutation({
		mutationFn: (reviewId: string) => apiReview.deleteReview(reviewId),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["vehicleReviews", id] });
			toast.show("Đánh giá đã được xóa thành công", "success");
		},
		onError: (error: any) => {
			toast.show(error.message || "Xóa đánh giá thất bại", "error");
		},
	});
		...
		return (
			<SafeAreaView className="flex-1 bg-white" edges={["top", "left", "right"]}>
				...
				<View className="flex-1 items-center justify-center px-4">
					...
					<Text className="mt-4 text-red-600 text-center">Không thể tải thông tin xe. Vui lòng thử lại.</Text>
				</View>
			</SafeAreaView>
		);
	}
	...
	const favoriteButton =
		!isOwner && user ? (
			<TouchableOpacity onPress={handleToggleFavorite} disabled={favoriteMutation.isPending} className="p-2">
				<MaterialIcons
	...
	return (
		<SafeAreaView className="flex-1 bg-white" edges={["top", "left", "right"]}>
				...
				<View className="px-4 pt-4">
					...
					<View className="mt-4 mb-4">
						<View className="flex-row items-start justify-between mb-2">
							<View className="flex-1">
								...
								{/* Thêm thống kê chuyến */}
								<View className="mt-2">
									<VehicleStats
										completedTrips={(vehicle as any).completedTrips ?? 0}
						...
						<View className="flex-row items-center mt-2">
							...
							<Text className="ml-2 text-base font-mono text-gray-700">{vehicle.licensePlate}</Text>
						</View>
					</View>
					...
					>
						<View className="flex-row items-center justify-between">
							<View>
								<Text className="text-sm text-gray-600 mb-1">Giá thuê/ngày</Text>
					...
					{/* Vehicle Details */}
					<View className="mb-4">
						<Text className="text-lg font-semibold text-gray-900 mb-3">Thông tin xe</Text>
						<View>
							<View className="flex-row items-center mb-2">
								<MaterialIcons name="settings" size={20} color="#6B7280" />
								<Text className="ml-3 text-base text-gray-700">Dung tích: {vehicle.engineSize} cc</Text>
							</View>
							<View className="flex-row items-center mb-2">
								<MaterialIcons name="directions-bike" size={20} color="#6B7280" />
					...
					{vehicle.description && (
						<View className="mb-4">
							<Text className="text-lg font-semibold text-gray-900 mb-2">Mô tả</Text>
							<Text className="text-base text-gray-700 leading-6">{vehicle.description}</Text>
						</View>
					)}
						...
						{isLoadingReviews ? (
							...
							<View className="space-y-3">
								{reviewsData.reviews.map((review: Review) => {
									const isMyReview = user?.id === review.authorId;
									return (
										<View
											key={review.id}
					...
					{isOwner && vehicle.status === "REJECTED" && (
						<TouchableOpacity
							onPress={() => router.push(`/(tabs)/profile/register-vendor?vehicleId=${vehicle.id}`)}
							className="bg-primary-600 rounded-xl p-4 mb-4"
						>
							<View className="flex-row items-center justify-center">
								<MaterialIcons name="edit" size={20} color="#FFFFFF" />
								<Text className="ml-2 text-base font-semibold text-white">Cập nhật và gửi lại</Text>
							</View>
						</TouchableOpacity>
					)}
			...
			{mapFullScreen && vehicle.lat != null && vehicle.lng != null && (
				<View style={localStyles.fullscreenOverlay}>
					...
					{/* 'Xem đường đi' button on full-screen map */}
					<View style={localStyles.directionsWrap}>
						<TouchableOpacity
							onPress={() =>
								openExternalMaps(
									Number(vehicle.lat),
									Number(vehicle.lng),
									`${vehicle.ward || vehicle.district || vehicle.city || vehicle.address || ""}`
								)
							}
							style={localStyles.directionsBtn}
							activeOpacity={0.9}
						>
							<MaterialIcons name="directions" size={18} color="#fff" />
							<Text style={localStyles.directionsText}>Xem đường đi</Text>
						</TouchableOpacity>
					</View>
				</View>
			)}
