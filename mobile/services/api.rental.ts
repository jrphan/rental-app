...
export interface Rental {
	...
	insuranceFee: string; //(string decimal from API)
}
...
export const apiRental = {
	async createRental(data: CreateRentalRequest): Promise<CreateRentalResponse> {
		const response = await apiClient.post<CreateRentalResponse>(API_ENDPOINTS.RENTAL.CREATE, data);
	...
	async getMyRentals(role?: "renter" | "owner" | "all", status?: RentalStatus): Promise<RentalListResponse> {
	...
	async updateRentalStatus(rentalId: string, data: UpdateRentalStatusRequest): Promise<UpdateRentalStatusResponse> {
		...
		throw new Error(response.message || "Cập nhật trạng thái đơn thuê thất bại");
	},
	...
	async createDispute(rentalId: string, data: CreateDisputeRequest): Promise<CreateDisputeResponse> {
