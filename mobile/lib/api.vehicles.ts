...
export const vehiclesApi = {
	...
	async getCities() {
		const res = await apiClient.get<any[]>("/cities");
		if (res.success) {
			if (Array.isArray(res.data)) return res.data;
			const wrapped = res.data as any;
			if (wrapped && Array.isArray(wrapped.data)) return wrapped.data;
		...
		throw new Error(res.message || "Lấy danh sách thành phố thất bại");
	},

	async createType(data: { name: string; description?: string; icon?: string }) {
	...
	async removeImage(vehicleId: string, imageId: string) {
		const res = await apiClient.delete(`/vehicles/${vehicleId}/images/${imageId}`);
